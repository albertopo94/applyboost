...
describe("indeedExtractor - extractViaMarkdownNew", () => {
  it("should send method: 'browser' in the POST body to markdown.new", async () => {
    ...
    const fetchSpy = spyOn(global, "fetch").mockImplementation(async () => {
      return new Response(JSON.stringify({ content: "Successful extraction with enough length for quality check to pass the 250 character limit if needed, but here we just want to verify the body of the request sent to the API." }), {
        status: 200,
  ...
  it("should return status 'ok' when content quality is good (> 250 chars)", async () => {
    ...
    const fetchSpy = spyOn(global, "fetch").mockImplementation(async () => {
      return new Response(JSON.stringify({ content: longMarkdown }), {
        status: 200,
        headers: { "x-rate-limit-remaining": "499" }
      });
    });
  ...
  it("should return status 'unreadable' when content quality is bad (< 250 chars)", async () => {
    ...
    const fetchSpy = spyOn(global, "fetch").mockImplementation(async () => {
      return new Response(JSON.stringify({ content: shortMarkdown }), {
        status: 200,
        headers: { "x-rate-limit-remaining": "499" }
      });
    });
